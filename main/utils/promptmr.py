...
class SensitivityModel(nn.Module):
    ...
    def get_pad_and_num_low_freqs(
        ...
        #if num_low_frequencies is None or num_low_frequencies == 0: # orj
        if num_low_frequencies is None or (num_low_frequencies == 0).all().item():
...
class PromptMR(nn.Module):
    ...
    def forward(
        ...
        r2=fastmri.rss(fastmri.complex_abs(fastmri.ifft2c(kspace_pred)), dim=1)
...
def complex_abs(data: torch.Tensor) -> torch.Tensor:
    ...
    #  data.shape: torch.Size([15, 640, 356, 2])
