...
class KneeRecon:
    def __init__(
        ...
        # load weights (handle Lightning-style ckpts too)
        sd = torch.load(ckpt_path, map_location="cpu",weights_only=False)
        if "state_dict" in sd:
            sd = sd["state_dict"]
            # optionally strip off any Lightning prefix:
        ...
        extra_keys = set(sd) - set(filtered_sd)
        if extra_keys:
            print(f"Ignoring unexpected checkpoint keys: {extra_keys}")
