...
from utils.transform_utils import normalize_instance
import numpy as np
...
def update_reward(base_logits, next_logits,base_probs, next_probs,slice_info, batch_ssim_base, batch_ssim):
    ...
    criterion = nn.CrossEntropyLoss(weight=slice_info['ce_weights'][0]).to(device)
...
def update_env(
    ...
    with torch.no_grad():
        ...
        normalized = []
        for img in recon_u:
            img_norm, _, _ = normalize_instance(img, eps=1e-11)
            normalized.append(img_norm)
        recon_norm = torch.stack(normalized, dim=0).to(args.device) 
        
        inp_out = recon_norm.unsqueeze(1)
        inp3=inp_out.expand(-1, 3, -1, -1).clamp(-6, 6)
